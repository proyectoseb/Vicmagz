...
class instagramGalleryImageHelper
{
     ...
     * @param mixed $image
     * @param array $options
     * @return instagramGalleryImageHelper
     */
    public static function init($image, $options = array())
    {
        $cacheid = count(self::$instances);
        if (!isset(self::$instances[$cacheid])) {
            self::$instances[$cacheid] = new instagramGalleryImageHelper($image, $options);
        }
        return self::$instances[$cacheid];
    }

    /**
     * instagramGalleryImageHelper constructor
    ...
    public function __construct($image = null, $options = array())
    {
        ...
        // defaults
        foreach (instagramGalleryImageHelper::$defaults as $name => $value) {
        ...
        $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::__construct');
    }
    ...
    {
        echo 'instagramGalleryImageHelper::$defaults' . '<br>';
    ...
    public function initCache($options = array())
    {
        ...
        if (empty($private_cache)) {
            if (is_null(instagramGalleryImageHelper::$cache) && defined('JPATH_CACHE')) {
                instagramGalleryImageHelper::$cache = JPATH_CACHE . '/resized';
                if (!file_exists(instagramGalleryImageHelper::$cache)) {
                    if (!@mkdir(instagramGalleryImageHelper::$cache, 0755, true)) {
                        instagramGalleryImageHelper::$cache = null;
                    }
                }
            }
            if (is_null(instagramGalleryImageHelper::$cache)) {
                instagramGalleryImageHelper::$cache = dirname(__FILE__) . '/resized';
                if (!file_exists(instagramGalleryImageHelper::$cache)) {
                    if (!@mkdir(instagramGalleryImageHelper::$cache, 0755, true)) {
                        instagramGalleryImageHelper::$cache = null;
                    }
            ...
            if (is_null(instagramGalleryImageHelper::$cache)) {
                ...
                $private_cache = instagramGalleryImageHelper::$cache;
            }
        }
    ...
    protected function getInputType()
    {
        if (!isset($this->in['type'])) {
            $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::getInputType');
    ...
    {
        $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::getInputResource');
    ...
    {
        $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::validOptions');
        // TODO: validate input
        $type = $this->getInputType();
        $input_type_ok = ($type == 'url') || in_array($type, instagramGalleryImageHelper::$supports);
        if (!$input_type_ok) {
        ...
        // required options
        $output_type_ok = !is_null($this->options['type']) && in_array($this->options['type'], instagramGalleryImageHelper::$supports);
        if (!$output_type_ok) {
            if ($input_type_ok) {
        ...
        if (in_array($this->options['function'], instagramGalleryImageHelper::$functions)) {
            if ($this->options['function'] == 'resize' && !in_array($this->options['function_mode'], instagramGalleryImageHelper::$modes)) {
                $this->errors[] = 'Resize mode is not valid! should be in [' . implode(', ', instagramGalleryImageHelper::$modes) . ']';
                $this->_process = 0;
            }
        } else {
            $this->errors[] = 'Output function is not valid! should be in [' . implode(', ', instagramGalleryImageHelper::$functions) . ']';
            $this->_process = 0;
        }
    ...
    protected function getOutputFile()
    {
        if (!array_key_exists('src', $this->out) || is_null($this->out['src'])) {
            $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::getOutputFile');
            ...
            $this->save2f();
            $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::getOutputFile::::' . $this->out['src']);
        }
        return $this->out['src'];
    }
    ...
    protected function getOutputUrl()
    {
        if (!isset($this->out['url'])) {
            $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::getOutputUrl');
            if ($this->validOptions() && in_array($this->options['type'], instagramGalleryImageHelper::$supports)) {
    ...
    public function output()
    {
        if ($this->validOptions() && $this->getInputResource()) {
            $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::output');
            $this->process();
    ...
    public function tag()
    {
        $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::tag');
    ...
    {
        $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::path');
        return $this->__toString();
    }

    public function src()
    {
        $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::src');
    ...
    {
        $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::error');
        return $this->errors;
    }
    ...
    {
        $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::resize');
    ...
    {
        $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::actionPerformed');
        ...
        if ($this->options['function'] == 'resize') {
            ...
            if ($ow == 0 && $oh == 0) {
                ...
                // resize by height.
                $this->options['function_mode'] = instagramGalleryImageHelper::$modes[3];
            ...
            switch ($this->options['function_mode']) {
                case instagramGalleryImageHelper::$modes[0]:
                    if ($ow > $dw) {
                        $dx = ($ow - $dw) / 2;
                    }
                    if ($oh > $dh) {
                    ...
                    break;
                case instagramGalleryImageHelper::$modes[1]:
                    $or = 1.0 * $sw / $sh;
                    if ($ow < $oh * $or) {
                        $dw = round($oh * $or);
                        $dh = $oh;
                    ...
                    break;
                case instagramGalleryImageHelper::$modes[2]:
                    $or = 1.0 * $sw / $sh;
                    if ($ow > $oh * $or) {
                        $dw = round($oh * $or);
                        $dh = $oh;
                    ...
                    break;
                default:
                case instagramGalleryImageHelper::$modes[3]:
                    $dw = $ow;
                    $dh = $oh;
                    break;
            }
...
// 		$this->save2f();
// 		$this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::__destruct');
// 	}

    protected function save2f()
    {
        $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::save2f');
    ...
    protected function getCanvas()
    {
        if (!isset($this->out['image'])) {
            $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::getCanvas');
            ...
            if ($this->options['transparency'] && in_array($this->options['type'], array(IMAGETYPE_GIF, IMAGETYPE_PNG))) {
                ...
                && $this->options['function'] == instagramGalleryImageHelper::$functions[0]
                && in_array($this->options['function_mode'], array(instagramGalleryImageHelper::$modes[0], instagramGalleryImageHelper::$modes[2]))
            ) {
    ...
    {
        $this->debug && JFactory::getApplication()->enqueueMessage('instagramGalleryImageHelper::__toString');

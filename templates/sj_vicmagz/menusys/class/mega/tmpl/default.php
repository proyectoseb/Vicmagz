...
if ( $this->isRoot() ){
	...
	//var_dump($fancyMenu);
	echo '<div class="mainmenu-wrapper hidden-sm hidden-xs"><ul id="'.$menucssid.'" class="'.$classfancyMenu.' clearfix navi hidden-sm hidden-xs '.($addCssRight=='rtl' ? ' navirtl':'').'">';
	if($this->haveChild()){
		$idx = 0;
				...
				?>
					<div class="mega-col<?php echo $addclass;?><?php echo count($mega_cols)>1?' more':' one'; ?>" <?php echo $styleWidth; ?>>
						...
						if($item_content_type=='menu'){
							...
							?>
								<div class="mega-module moduletable<?php echo $m_class_sfx;?> ">
									<?php if ($m_showtitle):?>
